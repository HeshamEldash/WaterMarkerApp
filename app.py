...
class App():
    ...
    def __init__(self):
        ...
        frm2 = ttk.Frame(self.root, height=1500, width=100)
        ...
        self.none_image = self.canvas.create_image(430, 430, image=None)
    ...
    def merge(self, im1, im2):
        ...
        top_corner = (int(coords[0] - im2_tk.width() / 2 + 30), int(coords[1] - im2_tk.height() / 2+30))
        print(f"coords of watermark {coords}")
        print(f"canvas size {self.canvas.winfo_height(), self.canvas.winfo_width()}")
        print(f"top corner {top_corner}")
    ...
    def position(self, event):
        ...
        x_edge = self.watermark_tk.width() / 2 + coords[0]
        y_edge = self.watermark_tk.height() / 2 + coords[1]


        if (
                self.x in range(int(x_edge) - 20, int(x_edge) + 20, 1) and
                self.y in range(int(y_edge) - 20, int(y_edge) + 20, 1)
        ):
            self.canvas.unbind("<B1-Motion>")
    ...
    def resize(self):
        if not self.flag:
            ...
            try:
                ...
                resdized = ImageTk.PhotoImage(self.watermark_object.resize((self.x, self.y), Image.Resampling.LANCZOS))
                self.watermark_tk = resdized
            except:
                pass
            self.watermark_object = self.watermark_object.resize((self.x, self.y), Image.Resampling.LANCZOS)
